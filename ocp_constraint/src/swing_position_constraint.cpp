...
namespace ocp_constraint {
  SwingPositionConstraint::SwingPositionConstraint(const ocp_solver::SwitchedModelReferenceManager& referenceManager,
  ...
  SwingPositionConstraint::SwingPositionConstraint(const SwingPositionConstraint& rhs)
    : StateConstraint(rhs),
  ...
  bool SwingPositionConstraint::isActive(ocs2::scalar_t time) const {
    bool inContact = referenceManagerPtr_->isInContact(time, endEffectorDynamicsPtr_->getFrameId());
  ...
  std::vector<std::pair<ocs2::scalar_t, pinocchio::SE3> > SwingPositionConstraint::nearestContact(ocs2::scalar_t time) const {
    ...
    for (size_t i=0; (i<contactSchedule.contactSequence.size()) && (afterTime == -1.0); i++) {
      for (std::pair<pinocchio::FrameIndex, pinocchio::SE3> contact : contactSchedule.contactSequence[i]) {
        if (contact.first == endEffectorDynamicsPtr_->getFrameId()) {
